$(document).ready(function () {
    function time() {
        ...
        var message = hours < 12 ? 'Günaydın' : hours < 18 ? 'İyi Akşamlar' : 'İyi Geceler';
    ...
    function cat() { // 0 pazar, 1 pazartesi, 2 salı, 3 çarşamba, 4 perşembe, 5 cuma, 6 cumartesi
        ...
        if (day === 1 || day === 2 || day === 3 || day === 4 || day === 5) {
            if (hours >= 8 && hours <= 16) {
                $("#cat").html('<i class="fa-regular fa-graduation-cap text-gray-500 inline-flex ml-1 mr-2"></i> Aktif');
            } else if (hours >= 23 || hours <= 8) {
                $("#cat").html('<i class="fa-regular fa-snooze text-gray-500 inline-flex ml-1 mr-2"></i> Uyuyor');
            } else {
                $("#cat").html('<i class="fa-regular fa-circle-question text-gray-500 inline-flex ml-1 mr-2"></i> Bir şey yapmıyor');
            }
        } else if (day === 6 || day === 0) {
            if (hours >= 23 || hours <= 8) {
                $("#cat").html('<i class="fa-regular fa-snooze text-gray-500 inline-flex ml-1 mr-2"></i> Uyuyor');
