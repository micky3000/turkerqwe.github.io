window.addEventListener('DOMContentLoaded', async function () {
  ...
  document.querySelectorAll('.github-card').forEach(async function (el) {
    ...
    el.innerHTML = `
            <div data-id="${data.full_name || 'NOT_FOUND'}" onclick="window.open('https://github.com/${data.full_name || turkerqwe}')" class="github mt-2 p-3 rounded-lg">
                <div class="github-left">
                    <div class="github-title">
                    <a style="color: gray !important">turkerqwe/</a>${data.name || "Türker"}
                    </div>
                    ...
                    <span>${data.language || "Undefined"}</span> <span class="ml-1 mr-1">•</span> <span>${data.stargazers_count || "0"}</span> Stars <span class="ml-1 mr-1">•</span> <span>${data.forks || "0"}</span> Forks
                    </div>
